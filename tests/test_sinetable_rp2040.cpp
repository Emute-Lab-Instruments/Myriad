...
static inline uint32_t get_cycles() {
    return time_us_32();  // Using microseconds as proxy for cycles
}
...
void test_performance() {
    ...
    uint32_t start_sw = get_cycles();
    for (int i = 0; i < ITERATIONS; i++) {
        volatile Q16_16 result = table.fast_sin(Q16_16(angle));
    }
    uint32_t end_sw = get_cycles();
    uint32_t time_sw = end_sw - start_sw;

    // Benchmark hardware version
    uint32_t start_hw = get_cycles();
    for (int i = 0; i < ITERATIONS; i++) {
        volatile Q16_16 result = table.fast_sin_hw(Q16_16(angle));
    }
    uint32_t end_hw = get_cycles();
    uint32_t time_hw = end_hw - start_hw;

    // Benchmark float version for comparison
    ...
    printf("  Iterations: %d\n", ITERATIONS);
    printf("  Software Fixed-Point: %lu µs (%.2f µs/call)\n",
           time_sw, (float)time_sw / ITERATIONS);
    printf("  Hardware Interpolator: %lu µs (%.2f µs/call)\n",
           time_hw, (float)time_hw / ITERATIONS);
    printf("  Float Lookup:         %lu µs (%.2f µs/call)\n",
           time_float, (float)time_float / ITERATIONS);
    printf("  std::sin (baseline):  %lu µs (%.2f µs/call)\n",
           time_std, (float)time_std / ITERATIONS);
    printf("\n");
    printf("  Speedup HW vs SW:     %.2fx\n", (float)time_sw / time_hw);
    printf("  Speedup HW vs Float:  %.2fx\n", (float)time_float / time_hw);
    printf("  Speedup HW vs std:    %.2fx\n", (float)time_std / time_hw);
}
...
void test_dual_hw() {
    ...
    printf("  Sequential HW: %lu µs\n", time_single);
    printf("  Dual HW:       %lu µs\n", time_dual);
