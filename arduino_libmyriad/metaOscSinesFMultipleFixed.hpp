...
/**
 * Fixed-Point Sine Wave Modulation Oscillator with Frequency Multiples
 *
 * Generates N independent sine waves with harmonic relationships.
 * Each oscillator runs at a frequency multiple of the base speed:
 * - Oscillator 0: 1x speed
 * - Oscillator 1: 2x speed
 * - Oscillator 2: 3x speed
 * - etc.
 *
 * Uses Q16.16 fixed-point arithmetic internally for better performance.
 *
 * Based on metaOscSinesFMultiple from metaOscs.hpp but with:
 ...
 */
template<size_t N>
class metaOscSinesFMultipleFixed {
public:
     ...
     *
     * Each oscillator i runs at (i+1) times the base speed, creating harmonic content.
     *
    ...
    std::array<FixedType, N> update(const size_t adcs[4]) {
        ...
        for(size_t i = 0; i < N; i++) {
            ...
            FixedType multipliedPhase = phasors[i] * FixedType(i + 1);

            // Lookup sine value using optimized table
            sines[i] = sine_table.fast_sin_hw(multipliedPhase) * moddepth;

            // Advance phase at base speed (all oscillators use same speed)
            phasors[i] += modspeed;
    ...
    const char* getName() const {
        return "speedy sines (fixed-point)";
    }
...
template<size_t N>
FixedPoint::SineTableQ16_16 metaOscSinesFMultipleFixed<N>::sine_table;
